...
const { AWSMarketplaceMeteringRecordsTableName: AWSMarketplaceMeteringRecordsTableName, AWS_REGION: aws_region } = process.env;
...
exports.handler = async (event) => {
  await Promise.all(event.Records.map(async (record) => {
    ...
    const { productCode, customerIdentifier } = body;
